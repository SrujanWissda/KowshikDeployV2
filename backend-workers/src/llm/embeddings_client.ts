...
export class GeminiEmbeddingsClient extends BaseEmbeddingsClient {
  ...
  // Workers has no global `process.env` — apiKey/model come from the caller
  // (Env secrets/vars) instead of being read here directly.
  constructor(apiKey: string | undefined, model: string = 'gemini-embedding-001') {
    super();
    this.apiKey = apiKey;
    this.model = model;
