...
// A single control test row nested inside TestEvidence.tests. Split out as its
// own schema (rather than inlined) because servicenow.ts's getControlEvidence()
// was attaching this shape via `as any` — undeclared, but real — and it now
// needs to be a properly typed, JSON-serializable shape since it crosses a
// Workflow step-return boundary (step.do() results must be plain serializable
// data, not an `any`-typed bag bolted on at runtime).
...
export const TestEvidenceSchema = z.object({
  ...
  openIssues: z.array(IssueSchema).default([]),
  closedIssues: z.number().default(0),
  tests: z.array(ControlTestSchema).default([])
});
