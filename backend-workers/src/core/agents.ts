    ...
    if (toAssess.length > 0) {
      ...
      for (const draft of drafts) {
        ...
        const auditTrail = [
          ...
          `Confidence: ${confidence}`,
          toolsUsedLine,
          `WHAT WAS INVESTIGATED (table-level detail):`,
          `&nbsp;&nbsp;1. Control details — investigated sn_compliance_control (control record) and found: "${item.controlName}"`,
          `&nbsp;&nbsp;2. Control tests — investigated sn_audit_control_test (Control Tests related list on the control) and found ${testCount} record${testCount !== 1 ? 's' : ''}: ${testDetailTech}`,
          `&nbsp;&nbsp;3. Associated issues — investigated sn_grc_issue (Issue Management module, same records as the Associated Issues tab on the control) and found ${allOpenIssues.length} record${allOpenIssues.length !== 1 ? 's' : ''} not yet Closed Complete: ${issueDetailTech}`,
          `&nbsp;&nbsp;4. Prior assessment history — investigated ${priorLineTech}`,
          `CONCLUSION:`,
          draft.justification,
          `Model: gemini-3.5-flash (WissdaSense) · Assessed: ${formattedDate}`
        ].join('<br><br>').replace(
  ...
  async execute(instanceSysId: string): Promise<{ success: boolean; message: string; details: any[] }> {
    ...
    for (const draft of drafts) {
      ...
      const comment = [
        ...
        '',
        'WHAT WAS INVESTIGATED:',
        `  1. ${entitySearchLabel} issues — investigated the ${searchTableLabel}; found ${issueCount} unresolved issue${issueCount !== 1 ? 's' : ''} not Closed Complete`,
        `  2. Risk-Related issues — ${issueRelevanceLine}`,
        '',
        'CONCLUSION:',
        draft.justification,
        '',
      ...
      const auditTrail = [
        ...
        toolsUsedLine,
        `WHAT WAS INVESTIGATED (table-level detail):`,
        `&nbsp;&nbsp;1. ${entitySearchLabel} issues — investigated ${techSearchTableLabel}; found ${issueCount} unresolved issue${issueCount !== 1 ? 's' : ''} not Closed Complete`,
        `&nbsp;&nbsp;2. Risk-Related issues — ${issueRelevanceLine}`,
        `CONCLUSION:`,
        draft.justification,
        `Model: gemini-3.5-flash (WissdaSense) · Assessed: ${formattedDate}`
