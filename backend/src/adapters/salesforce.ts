...
export class SalesforceAdapter extends BaseGRCAdapter {
    ...
    if (this.useLive) {
      try {
        ...
        await this.restUpdate('Risk__Control_Assessment__c', rowSysId, {
          ...
          Risk__Justification__c: `${justification}\n\nEvidence Summary:\n${evidenceSummary}`,
  ...
  // and writes AI recommendation narrative to the risk record
  // --------------------------------------------------------------------------
  async writeRiskControlMapping(
    ...
    if (this.useLive) {
      try {
        // ai_recommendation__c is a richtextarea field in Salesforce — format as clean HTML paragraphs and lists
        let recsHtml = '';
        if (recommendations) {
        ...
        console.log(`[Salesforce LIVE UPDATE] Updated grc__Risk__c ${riskSysId}.ai_recommendation__c with rich HTML mapping narrative`);

        // Second: Create control mapping junction records
        ...
        for (const ctrl of matchedControls) {
          try {
            const newId = await this.restCreate('Risk__Risk_Control_Lookup__c', {
              ...
              Risk__Control_Assessment_Justification__c: `${ctrl.reason}`.substring(0, 32768)
            });
            created.push(newId);
          } catch (innerErr: any) {
  ...
  private async createEMATrail(
    ...
    try {
      const token = await this.getAccessToken();
      // Ema_Audit_Summary__c is a plaintextarea field in Salesforce — convert HTML to clean formatted plain text
      const cleanSummary = convertHtmlToPlainText(auditTrailHtml);
        ...
        if (trailErr.response?.status === 404 || trailErr.message?.includes('sobject type')) {
          ...
          if (recordType === 'Risk__Control_Assessment__c') {
            await this.restUpdate(recordType, recordSysId, {
              Risk__Audit_Trail_Notes__c: cleanSummary.substring(0, 32768)
            });
  ...
  async writeObservabilityTrace(payload: {
    ...
    if (this.useLive) {
      const cleanSummary = convertHtmlToPlainText(payload.html || payload.summary || '');
